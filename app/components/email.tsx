...
const Email = ({ email }: { email: string }) => {
  ...
  const onSubmit: SubmitHandler<Schema> = async (data) => {
    ...
    try {
      // メールアドレス変更メールを送信
      const { error: updateUsereError } = await supabase.auth.updateUser(
        { email: data.email },
        // 確認ボタンを押下した際にリダイレクトさせるURL
        { emailRedirectTo: `${location.origin}/auth/login` }
      );

      // エラーチェック
      if (updateUsereError) {
        setMessage("エラーが発生しました。" + updateUsereError.message);
        return;
      }
      setMessage("確認用のURLを記載したメールを送信しました。");

      // ログアウト
      // (※ログアウトしないとcookieの情報と新しいメールアドレスとの整合性がとれなくなってしまうため)
