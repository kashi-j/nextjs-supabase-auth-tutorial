...
const Profile = () => {
  ...
  const onUploadImage = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const fileType = files[0]?.type; //Mine type of the file
  ...
  const onSubmit: SubmitHandler<Schema> = async (data) => {
    ...
    try {
      ...
      if (avatar) {
        ...
        // エラーチェック
        if (storageError) {
          setMessage("エラーが発生しました。3" + storageError.message);
          return;
        }
        if (avatar_url) {
      ...
      // エラーテェック
      if (updateError) {
        setMessage('エラーが発生しました。2' + updateError.message);
        return;
      }

      setMessage('プロフィールを更新しました。')
    } catch (error) {
      setMessage("エラーが発生しました。1" + error);
      return;
    } finally {
      setLoading(false);
      router.refresh();
